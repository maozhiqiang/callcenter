...
from src.task.management.callv1.psqlhelper import DBHelper

logger = Logger()

con = ESL.ESLconnection('localhost', '8021', 'Aicyber')
...
def event_processor(event_queue):
    ...
    while 1:
        ...
        if event_name in ['CHANNEL_HANGUP_COMPLETE']:
            ...
            logger.info('create sql : %s' % updatesql)
            db.runsql(updatesql)
        ...
        while 1:
            ...
            if e:
                ...
                if event_name in standard_event:
                    ...
                    if uuid in call_list:
                        call_list.remove(uuid)
            # time.sleep(.5)
    else:
...
def call_sender(call_list, call_size):
    ...
    while True:
        ...
        for i in range(min(call_free, len(uuid_list))):
            ...
            uuid = uuid_list.pop()
            #根据uuid　查找电话，并判断当前状态
            object  = db.getCustnumber_and_callStatus(uuid[0])
            logger.info("%s....%s"%(uuid[0],object))
            if object is not None:
                number = object[0]
                status = object[1]
                if cmp(status,'finishs')==0:
