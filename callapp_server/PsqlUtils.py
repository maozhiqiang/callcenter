...
DATABASE_HOST = conf.POSTGRESQL_HOST#"121.42.36.138" #要链接的服务器的IP地址，Server-IP
DATABASE_PORT = conf.POSTGRESQL_PORT#5432 #postgresql端口号，安装postgresql时设置的，一般默认是5432
DATABASE_NAME = conf.POSTGRESQL_NAME#"postgres" #数据库名称
DATABASE_USERNAME = conf.POSTGRESQL_USERNAME#"postgres" #用户名，这里的用户名也是在安装postgresql时设置的，一般默认postgres
DATABASE_PASSWORD = conf.POSTGRESQL_PASSWORD#"z8asuidn" #链接数据库服务器的密码，安装时设置，安装完毕后可以修改的
...
class DBHelper():
    ...
    # ----------------------new add ---------------------------------------------------------
    def getNumberById(self, uuid):
        sql = "select cust_number from fs_call where channal_uuid = '{0}'".format(uuid)
        self.cursor.execute(sql)
        print 'sql :', sql
        result = self.cursor.fetchone()
        if result is not None:
...
if __name__ == '__main__':
    ...
    list = db.getFlowIdAndAppId('15900282168','a6d24d2d-cebe-4113-9f89-9ce80012d6fc')
    print 'list...'
    if list is not None:
